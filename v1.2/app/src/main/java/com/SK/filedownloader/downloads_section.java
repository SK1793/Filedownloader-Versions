...
//            }
            ...
            switch (d_arr1[0].get_type().toLowerCase()){
                ...
                case "video":holder.ivFIle_icon.setImageResource(R.drawable.video_file);
                    break;
                case "image":holder.ivFIle_icon.setImageResource(R.drawable.image_file);
                    break;
                case "application":
                    if(d_arr1[0].get_extension().toLowerCase().contains("pdf")){
                        holder.ivFIle_icon.setImageResource(R.drawable.text_file);
                    }else if(d_arr1[0].get_extension().toLowerCase().contains("apk")){
                        holder.ivFIle_icon.setImageResource(R.drawable.apk_file);
                    } else if (d_arr1[0].get_extension().toLowerCase().contains("mp3") || d_arr1[0].get_extension().toLowerCase().contains("aac")
                            || d_arr1[0].get_extension().toLowerCase().contains("wav") || d_arr1[0].get_extension().toLowerCase().contains("wma") || d_arr1[0].get_extension().contains("aiff")
                            || d_arr1[0].get_extension().toLowerCase().contains("sbc") || d_arr1[0].get_extension().toLowerCase().contains("flac")) {
                        holder.ivFIle_icon.setImageResource(R.drawable.audio_file);
                    } else if (d_arr1[0].get_extension().toLowerCase().contains("mp4") || d_arr1[0].get_extension().toLowerCase().contains("3gp") || d_arr1[0].get_extension().toLowerCase().contains("h.")
                            || d_arr1[0].get_extension().toLowerCase().contains("av") || d_arr1[0].get_extension().toLowerCase().contains("vp9") || d_arr1[0].get_extension().toLowerCase().contains("avc")) {
                        holder.ivFIle_icon.setImageResource(R.drawable.video_file);
                    }else if(d_arr1[0].get_extension().toLowerCase().contains("octet")){
                        holder.ivFIle_icon.setImageResource(R.drawable.file);
                    }else if(d_arr1[0].get_extension().toLowerCase().contains("JPEG") || d_arr1[0].get_extension().toLowerCase().contains("JPG")
                    || d_arr1[0].get_extension().toLowerCase().contains("TIFF") || d_arr1[0].get_extension().toLowerCase().contains("PNG00")
                    || d_arr1[0].get_extension().toLowerCase().contains("SVG") || d_arr1[0].get_extension().toLowerCase().contains("HEIF")
                    || d_arr1[0].get_extension().toLowerCase().contains("JPEG") || d_arr1[0].get_extension().toLowerCase().contains("JPEG")){
                        holder.ivFIle_icon.setImageResource(R.drawable.image_file);
                    }else{
                        holder.ivFIle_icon.setImageResource(R.drawable.file);
                    }
                    break;
            ...
            if (tools.IsResumable(d_arr1[0].get_url())){

                holder.tvresumable.setText("Resumabble: Yes" );
            }else {

                holder.tvresumable.setText("Resumabble: No" );
            }
            ...
            holder.D_status.setText(tools.progressbarPercent(d_arr1[0].get_download_id())+"");
...
//            });Status_thread.start();
            executor.execute(()->{
             ...
             if (d_arr1[0].get_percent()!=null && !tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("fail") ||
                     tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("pend") ||
                tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("paus")){
...
//                holder.D_progress.setText("Progress: "+ tools.Percentage(d_arr1[0].get_download_id()));
            }else if(tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("fail")){
                 holder.btnPause.setEnabled(false);
                 holder.btnResume.setEnabled(false);
                 holder.D_status.setText(tools.progressbarPercent(d_arr1[0].get_download_id())+"");
                 holder.D_status.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.incomplete,0);
                 holder.D_progress.setText("Progress: "+tools.Status(d_arr1[0].get_download_id(),context));
                 holder.PbDProgress.setProgress(Math.abs(tools.progressbarPercent(d_arr1[0].get_download_id())));
             }
             else if(tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("pend")){
                 holder.btnPause.setEnabled(false);
                 holder.btnResume.setEnabled(false);
                 holder.D_status.setText(tools.progressbarPercent(d_arr1[0].get_download_id())+"");
                 holder.D_progress.setText("Progress: "+tools.Status(d_arr1[0].get_download_id(),context));
             ...
             else if(tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("pause")){
                 holder.btnPause.setEnabled(false);
             ...
             else if(tools.Status(d_arr1[0].get_download_id(),context).toLowerCase().contains("succ")){
                 holder.btnPause.setEnabled(false);
                 holder.btnResume.setEnabled(false);
                 holder.D_status.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.complete,0);
                 holder.D_status.setText(null);
...
//                    Looper.prepare();
//
//            if(!tools.Status(d_arr1[0].get_download_id(),context).contains("Successfull")){
...
//
//                    }else{
//                        Toast.makeText(context, " File Doesn't exist  "+d_arr1[0].get_filename(), Toast.LENGTH_SHORT).show();
//                    }
//
...
//                int removed=dm.remove(current_pos.get_download_id());
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        Removed[0] = dm.remove(item.get_download_id());
                        if (Removed[0] > 0) {
                            if (file.exists()) {
                                new Handler().post(() -> {
        ...
        final BroadcastReceiver onDownloadComplete = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (d_arr1[0].get_download_id() == downloadedID) {
                    holder.btnPause.setEnabled(false);
                    holder.btnResume.setEnabled(false);
                    notifyItemInserted(d_arr1[0]._size);
                }
            }
        };
//
...
//    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder implements  View.OnCreateContextMenuListener{
        ...
        CardView CVdelete;
        private TextView D_ID,D_name,D_ext,D_status,D_progress,D_Url,D_date,D_loc,D_type,D_size,D_Size_adapter,btnMore,tvresumable;
        RecyclerView Rv;
       ...
       public ViewHolder(@NonNull View itemView,Context context) {
           ...
           tvresumable=itemView.findViewById(R.id.tvResummable2);
